...
class dasboard(Frame):
    ...
    def setupUI(self):
        ...
        mydb.close()
        self.sep = Separator(self.container,orient='vertical').grid(row=0,rowspan=999,column=2,sticky='ns',padx=10)
        self.iter = 0
        self.clno = 0
        for txt in self.dasblabels.keys():
            ...
            self.tempLabel = Label(self.container, text=self.dasblabels[txt]).grid(column=self.clno + 1, row=self.iter, sticky='e', padx=8,pady=5)
            if self.iter == 4:
                self.iter = 0
                self.clno += 3
            else:
