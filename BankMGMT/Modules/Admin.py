...
class admin(Frame):
    def __init__(self,root, acno=None):
        ...
        for gen in self.genders:
            b = Radiobutton(self.genContainer,text=gen,value=gen,variable=self.gender)
            b.pack(side='left')
    ...
    def search(self,acno):
        threading.Thread(mycursor.execute(f"SELECT Uname,Nationality,Password,City,Address,AcType,Caste,MobileNo,Balance,DOB,Gender,Admin FROM profile WHERE AcNo = '{self.acnoen.get()}';")).start()
    ...
    def save(self):
        try:
            if not self.data:
                mycursor.execute(f"INSERT INTO profile(AcNo) VALUES('{self.acnoen.get()}');")
                mydb.commit()
        ...
        for values in self.labeldic.values():
            mycursor.execute(f"UPDATE profile SET {self.labels[iter_]}='{values.get()}' WHERE Acno in ('{self.acnoen.get()}');")
            iter_+=1
        mycursor.execute(f"UPDATE profile SET Gender ='{self.gender.get()}',Admin ='{self.isAdmin.get()}',DOB='{self.dob.get_date()}' WHERE Acno in ('{self.acnoen.get()}');")
        mydb.commit()
    def animateIn(self):
