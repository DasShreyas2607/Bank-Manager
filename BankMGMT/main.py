...
class App(Frame):
    ...
    def setupUI(self,master,theme):
        ...
        self.head = Label(self.headerFrame,text='SVK BANKING SYSTEMS',font=('Arial',18))
        self.ballb = Label(self.headerFrame,text='Balance:    ',font=('Arial',18))
        self.ballb2 = Label(self.headerFrame,textvariable=self.TkBal,font=('Arial',18))
    ...
    def panel(self):
        ...
        style.theme_settings(self.theme, {'TNotebook.Tab':{'configure':{'padding':[10,10]}}})
        self.notebook = Notebook(self.MainFrame,style='leftTab.TNotebook',width=800,height=500)
        ...
        self.Tkimages = {}
        self.ImageIter = 0
        for image in ['Dasboard','Transactions','Pay','Change Password','Settings','add_user','view','leave']:
        ...
        for tab in self.tabs.keys():
            self.notebook.add(self.tabs[tab],text=tab,image=self.Tkimages[tab.strip()],compound='top')
            self.ImageIter += 1

        if mycursor.fetchone()[0]:
            self.notebook.add(Admin.admin(self.notebook,self.AcNo),text='Add Ac',image=self.Tkimages['add_user'],compound='top')
            self.notebook.add(ViewCus.view(self.notebook),text='View Clients',image=self.Tkimages['view'],compound='top')

        self.notebook.add(Frame(),text='Logout',image=self.Tkimages['leave'],compound='top')
